...
scene_t *scene_create(int screen_width, int screen_height, float fov, SDL_Renderer *renderer) {
    ...
    scene->quad = quad;
    texture_t *quad_texture = texture_load("bin/appl/resources/smile.png");
    if (!quad_texture) {
        free(quad);
        SDL_free(camera);
        SDL_free(screen);
...
void _scene_draw_quad_texturized(scene_t *scene) {
    color_t blue = {0, 0, 255, 255};
    ...
    for (int i = 0; i < scene->quad->face_count; ++i) {
        ...
        cs_p1 = camera_world_to_camera_space(scene->camera, &cw_p1);
        cs_p2 = camera_world_to_camera_space(scene->camera, &cw_p2);
        cs_p3 = camera_world_to_camera_space(scene->camera, &cw_p3);

        vertex_t v1 = {
            &ss_p1, &COLOR_RED, cs_p1.z, (vector2f_t *)&(scene->quad->triangles[i].v1.uv),
        };
        vertex_t v2 = {
            &ss_p2, &COLOR_GREEN, cs_p2.z, (vector2f_t *)&(scene->quad->triangles[i].v2.uv),
        };
        vertex_t v3 = {
            &ss_p3, &COLOR_BLUE, cs_p3.z, (vector2f_t *)&(scene->quad->triangles[i].v3.uv),
        };
...
void _scene_draw_rotate_suzanne_with_scanline(scene_t *scene, float delta_time) {
    ...
    for (int i = 0; i < scene->suzanne->face_count; ++i) {
        ...
        vertex_t v1 = {
            &ss_p1, &COLOR_RED, cs_p1.z, (vector2f_t *)&(scene->suzanne->triangles[i].v1.uv),
        };
        vertex_t v2 = {
            &ss_p2, &COLOR_GREEN, cs_p2.z, (vector2f_t *)&(scene->suzanne->triangles[i].v2.uv),
        };
        vertex_t v3 = {
            &ss_p3, &COLOR_BLUE, cs_p3.z, (vector2f_t *)&(scene->suzanne->triangles[i].v3.uv),
        };
...
void scene_update(scene_t *scene, float delta_time) {
    ...
    _scene_draw_quad_texturized(scene);
...
void scene_destroy(scene_t *scene) {
    ...
    texture_destroy(scene->quad_texture);
